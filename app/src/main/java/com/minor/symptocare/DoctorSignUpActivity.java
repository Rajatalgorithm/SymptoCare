        ...
        auth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                ...
                if(task.isSuccessful())
                {
                                    ...
                                    public void onSuccess(Uri uri) {
                                        ...
                                        reference.child("Users").child(auth.getUid()).child("image").setValue(filePath).addOnSuccessListener(new OnSuccessListener<Void>() {
                                            ...
                                            public void onSuccess(Void unused) {
                                                Toast.makeText(DoctorSignUpActivity.this, "Write to database is successful", Toast.LENGTH_SHORT).show();
                                            }
                                        }).addOnFailureListener(new OnFailureListener() {
                                            @Override
                                            public void onFailure(@NonNull Exception e) {
                                                Toast.makeText(DoctorSignUpActivity.this, "An error occurred", Toast.LENGTH_SHORT).show();
                                            }
                                        });
                                    }
                                });
                    ...
                    Intent intent = new Intent(DoctorSignUpActivity.this, DoctorSignUpActivity2.class);
                    startActivity(intent);
                    finish();
                }
                else
                {
                    Toast.makeText(DoctorSignUpActivity.this, "There is a problem Signing up", Toast.LENGTH_SHORT).show();
                }
            }
        });
